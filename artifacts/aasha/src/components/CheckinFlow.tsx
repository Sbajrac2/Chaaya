    ...
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background/95 backdrop-blur-2xl">
      {typeof step === "number" && (
        <>
          ...
          <button onClick={skipAll} className="absolute top-8 right-6 flex items-center gap-1.5 text-white/25 hover:text-white/50 transition-colors">
            <span className="text-[10px] font-display tracking-widest uppercase">Skip</span>
            <SkipForward size={14} />
          </button>
        </>
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          <BinaryStep
            ...
            label="Showed up today?"
            subLabel="Did you make it to over 50% of your commitments?"
            yesIcon={<Check size={36} strokeWidth={2} />}
            noIcon={<X size={36} strokeWidth={1.5} />}
            yesTooltip="I showed up for my commitments"
            noTooltip="Couldn't make it today — whatever the reason is valid."
            onYes={() => { setAttendedClass(true); advance(2); }}
            onNo={() => { setAttendedClass(false); advance(2); }}
            onSkip={() => advance(2)}
            hoveredOption={hoveredOption}
        ...
        {step === 2 && (
          <BinaryStep
            ...
            icon={<Utensils size={48} strokeWidth={1} />}
            label="Nourished?"
            subLabel="Did you eat more than one meal today?"
            yesIcon={<Utensils size={32} strokeWidth={2} />}
            noIcon={<UtensilsCrossed size={32} strokeWidth={1.5} />}
            yesTooltip="I ate something today that felt like taking care of myself"
            noTooltip="Didn't eat well today."
            onYes={() => { setAteWell(true); advance(3); }}
            onNo={() => { setAteWell(false); advance(3); }}
            onSkip={() => advance(3)}
            hoveredOption={hoveredOption}
        ...
        {step === 3 && (
          <BinaryStep
            ...
            icon={<DoorOpen size={48} strokeWidth={1} />}
            label="Left your room?"
            subLabel="Did you socialize or engage with the outside world?"
            yesIcon={<DoorOpen size={32} strokeWidth={2} />}
            noIcon={<DoorClosed size={32} strokeWidth={1.5} />}
            yesTooltip="I stepped outside today — even briefly"
            noTooltip="Stayed in today — isolation is sometimes safety."
            onYes={() => { setLeftRoom(true); advance(4); }}
            onNo={() => { setLeftRoom(false); advance(4); }}
            onSkip={() => advance(4)}
            hoveredOption={hoveredOption}
        ...
        {step === 4 && (
          <motion.div key="masking" {...fadeVariants} className="flex flex-col items-center gap-10 w-full max-w-xs px-8">
            <div className="flex flex-col items-center gap-3 text-white/40">
              ...
              <p className="text-xs font-display tracking-[0.3em] uppercase text-white/30">Were you the real you today?</p>
              <p className="text-[10px] text-white/20 max-w-[240px] text-center leading-relaxed">The gap between who you are inside and who you showed the world.</p>
            </div>
            <div className="w-full space-y-5">
              <div className="relative">
                <div className="h-2 rounded-full bg-gradient-to-r from-emerald-500/60 via-violet-500/60 to-fuchsia-700/60 mb-2" />
                <input type="range" min={1} max={5} step={1} value={maskingLevel} onChange={(e) => setMaskingLevel(+e.target.value)} className="w-full absolute top-0 opacity-0 h-8 cursor-pointer" style={{ margin: "-12px 0" }} />
                <div className="w-7 h-7 rounded-full bg-white shadow-[0_0_20px_rgba(255,255,255,0.4)] absolute top-1/2 -translate-y-1/2 -translate-x-1/2 pointer-events-none transition-all duration-150" style={{ left: `${((maskingLevel - 1) / 4) * 100}%` }} />
              </div>
              <div className="flex justify-between text-[10px] font-display tracking-widest text-white/30 uppercase">
                <span>Fully you</span>
                <span>Heavily masked</span>
            ...
            <div className="flex gap-3">
              {[1, 2, 3, 4, 5].map((v) => (
                <button key={v} onClick={() => setMaskingLevel(v)} className={cn("w-8 h-8 rounded-full border transition-all duration-200", maskingLevel === v ? "bg-white/80 border-white/80 scale-110" : "bg-white/10 border-white/20 hover:bg-white/20")} />
              ))}
            </div>
            <button onClick={() => submit(maskingLevel)} className="mt-2 px-10 py-3.5 rounded-full bg-white/10 border border-white/20 text-white font-display text-sm tracking-widest uppercase hover:bg-white/20 active:scale-95 transition-all">Done</button>
          </motion.div>
        )}
        ...
        {step === "bio" && bioCard && (
          <motion.div key="bio" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col items-center gap-8 px-8 max-w-sm text-center">
            <FactTypeIcon factType={bioCard.factType} />
            <div className="space-y-4">
              <p className="text-base font-sans font-light text-white/85 leading-relaxed">{bioCard.card}</p>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-violet-500/15 border border-violet-500/20 text-[10px] text-violet-300 tracking-widest uppercase">+{bioCard.xpGained} Wisdom XP</div>
            </div>
            <button onClick={() => onComplete()} className="px-8 py-3 rounded-full bg-white/10 border border-white/15 text-white/60 font-display text-xs tracking-widest uppercase">Continue</button>
          </motion.div>
        )}

        {step === "done" && (
          <motion.div key="done" {...fadeVariants} className="flex flex-col items-center gap-8 px-10 max-w-sm text-center">
            <div className="w-20 h-20 bg-accent rounded-full animate-pulse shadow-[0_0_20px_hsl(var(--accent))]" />
            <p className="text-lg font-sans font-light text-white/90">Petal recorded.</p>
          </motion.div>
        )}
      </AnimatePresence>
...
function BinaryStep({ icon, label, subLabel, yesIcon, noIcon, yesTooltip, noTooltip, onYes, onNo, onSkip, hoveredOption, setHoveredOption }: any) {
  return (
    <motion.div {...fadeVariants} className="flex flex-col items-center gap-8">
      <div className="flex flex-col items-center gap-2 text-white/40">
        {icon}
        <p className="text-xs font-display tracking-[0.3em] uppercase text-white/30">{label}</p>
        {subLabel && <p className="text-[10px] text-white/15 tracking-wide text-center max-w-[200px]">{subLabel}</p>}
      </div>
      <div className="flex gap-8">
        <motion.button whileTap={{ scale: 0.95 }} onClick={onYes} onMouseEnter={() => setHoveredOption("yes")} onMouseLeave={() => setHoveredOption(null)} className="rounded-full border border-violet-500/40 bg-violet-500/10 text-violet-300 flex items-center justify-center w-[88px] h-[88px] transition-colors hover:bg-violet-500/20">{yesIcon}</motion.button>
        <motion.button whileTap={{ scale: 0.95 }} onClick={onNo} onMouseEnter={() => setHoveredOption("no")} onMouseLeave={() => setHoveredOption(null)} className="rounded-full border border-white/15 bg-white/5 text-white/40 flex items-center justify-center w-[88px] h-[88px] transition-colors hover:bg-white/10">{noIcon}</motion.button>
      </div>
      <AnimatePresence>
        {hoveredOption && <motion.p initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-[11px] text-white/35 max-w-[280px] text-center leading-relaxed">{hoveredOption === "yes" ? yesTooltip : noTooltip}</motion.p>}
      </AnimatePresence>
      <button onClick={onSkip} className="text-[10px] text-white/20 uppercase tracking-widest hover:text-white/40 transition-colors">Skip this</button>
    </motion.div>
  );
}
...
function FactTypeIcon({ factType }: { factType: string }) {
  ...
  return <div className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center">{iconMap[factType] || iconMap.general}</div>;
}